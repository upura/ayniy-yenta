...
def add_user_ages(dataframe: pd.DataFrame) -> pd.DataFrame:
    df_user_ages = pd.read_csv(INPUT_DIR + "user_ages.csv",
                               dtype={'user_id': str, 'age': float})
...
def add_user_purposes(dataframe: pd.DataFrame) -> pd.DataFrame:
    df_user_purposes = pd.read_csv(INPUT_DIR + "user_purposes.csv",
                                   dtype={
                                       'user_id': str,
                                       'purpose_id_1': int,
                                       'purpose_id_2': int,
    ...
    _dataframe = dataframe.copy()
    _dataframe = pd.merge(_dataframe,
                          df_user_purposes,
                          left_on="userA",
                          right_on="user_id",
                          how="left")
...
if __name__ == "__main__":
    ...
    if IS_TRAIN:
        ...
        Data.dump(train.drop(
            ['from-to', 'score', 'userA', 'userB', 'user_id_userA', 'user_id_userB'],
            axis=1
        ...
        Data.dump(test.drop(
            ['from-to', 'userA', 'userB', 'user_id_userA', 'user_id_userB'],
            axis=1
